...
ex2_questions_bysubject <- ex2_questions_bysubject_rt %>%
  group_by(subject_id, group) %>%
  summarise(mean_rt = mean(rt)) %>%


ex2_questions_bysubject <- results %>%
  group_by(subject_id, group) %>%
  summarise(dprime = dprime)
...
ggplot(results, aes(x = group, y = dprime, color = group)) + 
  geom_col() + 
  labs(x = "Groups", y = "DPrime", title = "Performance by Group")
